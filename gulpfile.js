...
const csscomb = require('gulp-csscomb');              //sort css 
...
function comb(){
	return gulp
	.src(['src/scss/**/*.scss'], {base: './'})
	.pipe(csscomb())
	.pipe(gulp.dest('./'))
	.pipe(gulp.src(['src/css/**/*.css', '!src/css/lib/**/*'], {base: './'}))
	.pipe(csscomb())
	.pipe(gulp.dest('./'));
}
...
function media() { //copy any miscellaneous media files over
	return gulp
	.src('src/media/**/*')
